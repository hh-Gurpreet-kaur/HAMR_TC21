...
export default class userLoginScreen extends React.Component {
  constructor (props) {
    ...
    this.tryGuest = this.tryGuest.bind(this)
  }
  ...
  setSecurityLevel (secLevel) {
    const permissions = secLevel.split(',')

    permissions.forEach((permission) => {
      const level = global.securityLevel
  ...
  getIntegrated () {
    if (config.debugIntegrated) {
      ...
      const query =
        "SELECT SettingValue FROM Settings WHERE SettingName='IsThirdParty'"
  ...
  async tryLogin () {
    ...
    if (oneTest) {
      const query = ` SELECT * FROM Employee WHERE Employee.Login = '${user}' `
      ...
      sqldb.executeReader(query).then((results) => {
        ...
        if (len == 1 && dbPass == pass) {
          this.setState({
            ...
            passwordError: false,
            username:
              this.state.rememberToggle || config.easyAccess ? user : '',
    ...
    if (this.state.usernameError) {
      return (
        <View style={style.passwordError}>
          <Text style={style.errorText}>{translate('username_wrong')}</Text>
        </View>
      )
    }
  }
  ...
  renderPasswordError = () => {
    if (this.state.passwordError) {
      return (
        <View style={style.passwordError}>
          <Text style={style.errorText}>{translate('password_wrong')}</Text>
        </View>
      )
    }
  }
        ...
        <View style={style.container}>
          <View style={style.header}>
            <View style={style.headerIcon}>
              <Image
                source={require('../../assets/img/hhIcon.png')}
                style={style.hhIcon}
              />
            </View>
          ...
          <View style={style.loginBox}>
            <View style={style.login}>
              ...
              <View style={style.remember}>
                <View style={style.checkView}>
                  <CheckBox
                    value={this.state.rememberToggle}
                    onValueChange={this.toggleRemember}
                  />
                  <TouchableNativeFeedback>
                ...
                <View style={style.help}>
                  <Icon
                    ...
                    size={20}
                    color={'black'}
                    onPress={() => alert(translate('remember_login'))}
                  />
                </View>
              </View>
            </View>
            ...
            <View style={style.loginButton}>
              <AwesomeButton
                ...
                textColor={'white'}
                borderColor={color.light_green_darker}
                onPress={this.tryGuest}
              >
                <Text
                  style={{ fontSize: 26, color: 'white', fontWeight: 'bold' }}
                >
                  {translate('enable_guest')}
                </Text>
              </AwesomeButton>
            </View>
          </View>

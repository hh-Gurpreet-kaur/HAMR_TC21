...
class OrderInfo extends React.Component {
	...
	getData() {
		...
		let booth = '';
		let supplierName = '';
		let qOnOrder = `SELECT COUNT(*) as Count FROM Market`;
		let qTotalOrders = `SELECT SkuNum from MarketOrder UNION SELECT SkuNum from Market WHERE OrderQty != 0`;
		let qModifiedOrders = `SELECT COUNT(*) as Count FROM MarketOrder`;

		let sku = this.props.navigation.getParam('sku', '');
		let qBooth = `SELECT Market.Booth, 
		...
		sqldb.executeReader(qModifiedOrders).then((results) => {
			...
			this.setState({modifiedOrders: modifiedOrders})
		})
...
export default addHeader(OrderInfo, 'Market Order')
...
class QuickEntry extends React.Component {
  ...
  start = () => {
    const q = `SELECT MarketOrder.* , ifnull(Market.Description, '${translate('not_on_file')}') as Description FROM MarketOrder
  ...
  deleteRow = (sku) => {
    const q = `DELETE FROM MarketOrder WHERE SkuNum = ${sku}`
    sqldb.executeQuery(q)
    this.state.selectedLine = null
  }

  updateCheck = (sku, quantity) => {
    const selected = this.state.quantityCorrect ? 1 : 0
    const testQuery = `SELECT * FROM MarketOrder WHERE SkuNum = ${sku}`

    const qty = EditFactor.AdjustQuantity(this.state.item, quantity)
    ...
    sqldb.executeReader(testQuery).then((results) => {
      ...
      if (len == 0) {
        ...
        const updateQ = `UPDATE MarketOrder SET Qty = ${qty}, Selected = ${selected} WHERE SkuNum = ${sku}`
  ...
  addItem = (sku, quantity, selected) => {
    if (sku.length > 0 && quantity != '') {
      const query = `INSERT INTO MarketOrder(SkuNum, Qty, Selected) VALUES(${sku}, ${quantity}, ${selected})`
  ...
  getQty = (sku) => {
    const query = 'SELECT MarketOrder.Qty, Market.OrderQty, MarketOrder.Selected' +
		',ItemMaster.BuyConv' +
  ...
  getSkuFromUpc (upc) {
    const query = 'SELECT SkuNum FROM Upc WHERE upc}'
  ...
  changeItemNumber = async (scan) => {
    ...
    else if (scan.length < 11 || scan.length >= 11) {
...
export default addHeader(QuickEntry, 'market_order')

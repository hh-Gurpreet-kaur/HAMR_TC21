...
export default class Work extends React.Component {
	...
	async processBlock(contents, lastRead) {
		...
		if(typeof contents === 'string') {
			...
			while(contents.length > 0) {
				// get line
				let line = ''
				let n = contents.indexOf('\n')
				line = contents.slice(0, n)

				// if COMMENT
				let comment = (line[0] == '-' && line[1] == '-')
				if(comment) {
				...
				// if CREATE
				let create = line.indexOf('CREATE') == 0
				if(create) {
					// console.log('CREATE found!')

					// find end of command
					let createEnd = contents.indexOf(';')
					...
					// get whole command
					let cmd = contents.slice(0, createEnd+1)
				...
				// if INSERT + REPEAT
				let insertAndRepeat = line.indexOf('INSERT') == 0
				if(insertAndRepeat) {
					...
					let addCheck = contents[insertEnd+3] == '\n'
					...
					// get whole cmd
					let insertCmd = contents.slice(0, insertEnd + 1)
					...
					// 2. newline after '('
					let repeat = contents.indexOf('REPEAT (') == 0
					let repeatAddCheck = repeat && contents[9] == '\n'
					...
					if (repeatAddCheck) {
						...
						while(1) {
							...
							let nend = contents.indexOf('\n')
							if (nend == -1) {
								// need more READ :(

								let path = RNFS.DocumentDirectoryPath + '/ex1.tmp'
	...
	async begin() {
		...
		// get file size
		let res = await RNFS.stat(testPath)
		size = res.size
		// console.log(size)
	...
	process(contents) {
		if(typeof contents === 'string') {
			...
			while(contents.length > 0) {
				let pos = contents.search(';')
				let cmd = contents.substr(0, pos+1)
				buf.push(cmd)
	...
	async singleQuery(q) {
		let db = await sql.openDatabase("Home.db","1.0","HH DB", 2000000 ,this.openCB, this.errorCB)
		try {

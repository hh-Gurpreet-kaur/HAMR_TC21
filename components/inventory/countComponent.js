...
export default class MainCount extends React.Component {
    ...
    clearItem = (saveScan) => {
		let scan = saveScan ? saveScan : "";
		let bg = saveScan ? color.invalid_scan : 'white';
		this.setState({
			barcodeBg: bg,
			itemNumber: "",
			description: "",
		...
		sqldb.executeReader(query).then((results) => {
			...
			if (len > 0) {
				...
				if (type == "sku") {
					...
					let existsQuery = `SELECT Qty FROM [InventoryCount] WHERE UpcCode='${upc}'`;
					let qty = "";
					let update = false;
    ...
    addBySku = () => {
        this.props.addBySku(this.state.itemNumber, this.state.quantity, this.state.description);
    }

	addByUpc = () => {      
        this.props.addByUpc(this.state.previousSku, this.state.quantity, this.state.description);
	}
        ...
        return(
            <View style={{width: "100%", alignSelf: 'center', paddingHorizontal: 10}}>
				...
				<View style={invStyle.descBox} >
					<Text style={invStyle.description}>
				...
				<View style={{flexDirection: "row", justifyContent: 'space-between', alignItems: 'center'}}>
					...
					<AwesomeButton 
						...
						textSize={22} width={150} height={50}
						textColor={'white'}
						borderColor={color.light_green_darker}
						onPress={this.onCountPressed}> 
						<Text style={{fontSize: 20, color: "white", fontWeight: "bold"}}>{translate("count_btn")}</Text>
					</AwesomeButton>
				</View>
				...
				<View style={invStyle.deletePrintPanel}>
					<AwesomeButton 
						...
						<Text style={{fontSize: 19, color: "white", fontWeight: "bold"}}>{translate("print_btn")}</Text>
					</AwesomeButton>
					<AwesomeButton 
						backgroundColor={color.pink} 
						backgroundDarker={color.pink_darker} 
						...
						<Text style={{fontSize: 19, color: "white", fontWeight: "bold"}}>{translate("delete_btn_caps")}</Text>
					</AwesomeButton>
				</View>
				<DeleteModal
					onDeleteRow={this.pressDelete}
					prompt={translate('delete_line_prompt', { itemNumber: this.state.scan })}
		...
		return (
			<View style={styleRow.rows}>
				...
				<View style={styleRow.column}>
					...
					<View style={styleRow.previousUpc}>
						<Text style={[styleRow.defaultText, {textAlign: 'center'}, props.valueStyleRight]}> {props.valueRight} </Text>
					</View>
				</View>
		...
		return (
			<View style={styleRow.rows}>
				...
				<View style={styleRow.column}>
					...
					<View style={styleRow.previousUpc}>
						<Text style={[styleRow.defaultText, {textAlign: 'center'}]}> {props.valueRight} </Text>
					</View>
				</View>

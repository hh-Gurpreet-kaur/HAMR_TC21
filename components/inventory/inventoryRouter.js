...
class InventoryRouter extends React.Component {
	...
	componentDidMount() {
		...
		.then((x) => {
			db = x
			this.getCounts();
			this.focusListener = this.props.navigation.addListener("didFocus", this.getCounts)
		})
	...
	getCounts = () => {
		let qFull = `SELECT COUNT(*) as Count FROM InventoryCount WHERE Tag IS NOT NULL AND Location IS NULL`;
		let qCycle = `SELECT COUNT(*) as Count from InventoryCount WHERE Tag IS NULL AND Location IS NOT NULL`;
		let qSpot = `SELECT COUNT(*) as Count FROM InventoryCount WHERE Tag IS NULL AND Location IS NULL`;
		db.transaction((tx) => {
	...
	renderCycleCount(item) {
		if (global.isThirdParty) {
			if (this.state.currType == "" || this.state.currType == "cycle") {
				return (
					<Item title={item[1].key} body={item[1].description} icon={item[1].icon} iconLib={item[1].iconLib}
						onPress = {() => this.props.navigation.navigate('CycleCount_Inventory') } />
				)
			} else {
				return (
					<Item title={item[1].key} body={item[1].description} icon={item[1].icon} iconLib={item[1].iconLib} iconColor={'#608060'} back={'lightgrey'} disabled={true}
	...
	renderFullCount(item) {
		if (this.state.currType == "" || this.state.currType == "full") {
			return (
				<Item title={item[0].key} body={item[0].description} icon={item[0].icon} iconLib={item[0].iconLib}
					onPress = {() => this.props.navigation.navigate('FullCount_Inventory') } />
			)
		} else {
			return (
				<Item title={item[0].key} body={item[0].description} icon={item[0].icon} iconLib={item[0].iconLib} iconColor={'#608060'} back={'lightgrey'} disabled={true}
					onPress = {() => alert(translate('count_in_progress')) } />
			)
		}
	...
	renderSpotCheck(item) {
		if (this.state.currType == "" || this.state.currType == "spot") {
			return (
				<Item title={item[2].key} body={item[2].description} icon={item[2].icon} iconLib={item[2].iconLib}
					onPress = {() => this.props.navigation.navigate('SpotCheck_Inventory') } />
			)
		} else {
			return (
				<Item title={item[2].key} body={item[2].description} icon={item[2].icon} iconLib={item[2].iconLib} iconColor={'#608060'} back={'lightgrey'} disabled={true}
					onPress = {() => alert(translate('count_in_progress'))} />
			)
		}
	}

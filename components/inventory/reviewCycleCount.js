...
class ReviewCycleCount extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      tableHead: [
  ...
  getResults = () => {
    const q = `SELECT [InventoryCount].InventoryCountID as Id, [InventoryCount].SkuNum as SkuNum, 
		...
		[ItemMaster].Description as Description FROM [InventoryCount] 
			...
			AND [InventoryCount].Tag IS NULL AND [InventoryCount].Location IS NOT NULL
			UNION
  ...
  render () {
    const pageCaption = translate('cycle_count_tab')
...
export default addHeader(ReviewCycleCount, 'inventory')

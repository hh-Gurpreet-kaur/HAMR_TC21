...
class MainCycleCount extends React.Component {
  ...
  getStoreSections = () => {
    const query = 'SELECT ItemLocationID, RetailLocation from ItemLocation'
  ...
  addBySku = (sku, quantity, description) => {
    ...
    query = `INSERT INTO [InventoryCount](SkuNum,Qty,EmployeeID,Location,StartDate)
		VALUES (${sku},${quantity.toString()},${global.employeeId},'${
  ...
  addByUpc = (upc, quantity, description) => {
    ...
    query = `INSERT INTO [InventoryCount](UpcCode,Qty,EmployeeID,Location,StartDate)
			VALUES (${upc},${quantity.toString()},${global.employeeId},'${
      this.state.selectedSection
    }','${startDate}')`
  ...
  timedPrintModal = () => {
    setTimeout(this.togglePrintModal, 3000)
  }
    ...
    if (this.state.selectedSection == '') {
      return (
        <View style={{ width: '95%', alignItems: 'center' }}>
          <Text style={invStyle.headingCenter}>

  ...
  onConnect = async () => {
    ...
    if (regMacAddress.test(this.state.printerAddress)) {
      if (!(await Bluetooth.BluetoothEnabled())) {
        ...
        success = await Bluetooth.Connect(
          this.state.printerAddress.toUpperCase()
        );
        ...
        <View style={style.container}>
          ...
          <View style={style.sectionLeft}>
            ...
            <Text style={style.subtitle}>
              {translate("connect_printer_desc")}
            </Text>
            <View
              style={{
                width: "100%",
          ...
          <View style={style.sectionLeft}>
            ...
            >
             ...
             <Text style={style.title}>{translate("printer_type")} </Text>
            <Picker
              selectedValue={this.state.printType}
              onValueChange={(val) => {
                this.savePrintType(val);
              }}
              style={style.pick}
            >
              <Picker.Item label={translate("zebra_printer")} value="zebra" />
              <Picker.Item label={translate("PB32_printer")} value="pb32" />
            </Picker>  
          ...
          <View style={style.sectionCenter}>
            <AwesomeButton
              ...
              width={220}
              height={40}
              onPress={this.onSendFonts}
            >
              <Text style={{ color: "white", fontWeight: "bold" }}>
                {translate("send_fonts")}
              </Text>
            </AwesomeButton>
          </View>
        </View>

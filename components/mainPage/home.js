	...
	constructor(props) {
		...
		this.state = {
			routeData : [
				...
				{
					key: 'Sync',
					description: 'Get connection information, sync your \n images and view the last sync date',
					icon: 'folder-sync', 
				},
				{
...
export class Item extends React.Component {
	...
	renderIcon() {
		...
		if (this.props.iconLib == 'FontAwesome') {
			return (
				<FontAwesome name={this.props.icon} size={17} color={color} />
			)
		}
		else if (this.props.iconLib == 'Feather') {
			return (
				<Feather name={this.props.icon} size={27} color={color} />
			)
		}
		else {
			return (
				<IconMC name={this.props.icon} size={22} color={color} />
			)
		}
	}
		...
		if (this.props.body) {
			if (this.props.disabled) {
				return(
					...
					<View style={[style.outerLi, this.backCircle, {backgroundColor: back}] } >
						...
						<View style={style.textLi}>
							<View style={style.title}>
								<Text style={[style.titleLi, {color: 'grey'}]}>{this.props.title}</Text>
							</View>
							<View style={style.subtitle}>
								<Text style={[style.bodyLi, {color: 'grey'}]}>{this.props.body}</Text>
							</View>
						</View>

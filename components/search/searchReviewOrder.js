...
class ReviewOrder extends React.Component {
	...
	getResults = () => {
		let q = `SELECT [Order].OrderID, [Order].SkuNum, [Order].Qty, ItemSupplier.AcctCost, 
	...
	onPrint = async () => {
		if (this.state.selected != null) {
			...
			let selectedItem = this.state.tableData[this.state.selected];
			let modalText = "";
			...
			if (Settings.labelType == "shelf") {
				printSuccess = await Label.PrintLabel("shelf", selectedItem.key, Settings.printerAddress);
				modalText = "shelf";
			} else {
				printSuccess = await Label.PrintLabel("upc", selectedItem.key, Settings.printerAddress);
				modalText = "UPC"
			}
	
			if (printSuccess === true) {
				this.setState({
					modalText: translate('printing_label', {labelType: modalText, itemNumber: selectedItem.key})
				})
				this.timedModal();
			} else {
				alert(printSuccess);
	...
	toOrder = () => {
		if (this.state.selected != null) {
			this.props.navigation.navigate('OrderScreen', {sku: this.state.tableData[this.state.selected].key});
		} else {
		...
		return(
			<View style={{flex: 1}}>
				...
				<View style={[style.orderPrintPanel]}>
					...
					<TouchableNativeFeedback background={TouchableNativeFeedback.Ripple(color.btn_selected)} onPress={() => this.props.navigation.goBack()}>
						<View style={style.btn}>
...
export default addHeader(ReviewOrder, 'Review Order')
...
class SearchResults extends React.Component {
	...
	render() {
		let numResults = this.state.tableData.length
		...
		<View style={style.container}>
			...
			<View style={style.head}>
				...
				<View style={style.resultsBox}>
					<Text style={style.resultsText}> {numResults} {translate("results")} </Text>
					<LoadingIndicator count={this.state.tableData.length} />
				</View>
			</View>
			<View>
			<View style={{ backgroundColor: '#b4bbbf', flexDirection: "row" }}>
				<TouchableNativeFeedback onPress={() => { this.sortResults(1) }}>
					<View style={[rowStyle.cell, { width: 100 }]}>
						<Text style={[rowStyle.cellText, { fontWeight: 'bold' }]}> {this.state.tableHead[0]} </Text>
					</View>
				</TouchableNativeFeedback>

				<TouchableNativeFeedback onPress={() => { this.sortResults(2) }}>
					<View style={[rowStyle.cell, { width: 65, alignItems: 'center' }]}>
						<Text style={[rowStyle.cellText, { fontWeight: 'bold' }]}> {this.state.tableHead[1]} </Text>
					</View>
				</TouchableNativeFeedback>
				<TouchableNativeFeedback onPress={() => { this.sortResults(3) }}>
					<View style={rowStyle.cell}>
						<Text style={[rowStyle.cellText, { fontWeight: 'bold' }]}> {this.state.tableHead[2]} </Text>
					</View>
				</TouchableNativeFeedback>
			</View>
			</View>
...
export default addHeader(SearchResults, 'Search Results')

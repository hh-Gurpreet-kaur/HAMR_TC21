...
class Order extends React.Component{
	...
	componentDidUpdate(prev) {
		let refresh = prev.navigation.getParam('state','') != this.props.navigation.getParam('state','')
		if(refresh) {
			this.initDb()
		}	
	}
		...
		sqldb.executeReader(searchQ).then((results) => {
			if (results.length == 0) {
				...
				if (x.HomeSource != "H") {
					...
					// pre calculate
					let price = x.RetailPrice > 0 ? x.RetailPrice : x.HomeRetailPrice
					let gm = 0;
					...
					this.setState({
						...
						shipCode: x.shipCode,
						minMax: x.Min + '/' + x.Max,
						velocityCode: x[`ifnull(ItemMaster.Velocity,'///')`] + x[`ifnull(ItemMaster.HomeVelocity,'')`],
					...
					let orderCheck = `SELECT * FROM [Order] WHERE SkuNum = ${this.state.itemNumber}`
					sqldb.executeReader(orderCheck).then((results) => {
						if (results.length == 1) {
							// existing order, get quantity and update
							let oldQty = results.item(0).Qty + 1;
							this.setState({
	...
	start = () => {
		let prev = this.props.navigation.getParam('state', 1)
		if (prev === 1) {
			return
		}
		...
		if (prev.homeSource != "H") {
			...
			let orderCheck = `SELECT * FROM [Order] WHERE SkuNum = '${prev.itemNumber}'`
			sqldb.executeReader(orderCheck).then((results) => {
				if (results.length == 1) {
					// existing order, get quantity and update
					let oldQty = results.item(0).Qty + 1;
	...
	handleItem = async (scan, isScan) => {
		...
		if (scan == this.state.previousSku || scan == this.state.previousBarcode) {
			...
			if (scan.indexOf('^') != -1) {
				...
				let tmpScn = scan.substring(1);
				if (tmpScn.length >= 11)
				{
					let upc = await Scanning.checkHomeUpc(tmpScn);
					if (upc.length == 7) {
						this.findBySku(upc);
					} else {
						this.findByUpc(upc);
				...
				if (scan.length >= 11) {
					...
					let upc = await Scanning.checkHomeUpc(scan);
	...
	onPrint = async () => {
		...
		if (printSuccess === true) {
			this.setState({
				printingText: translate('printing_label', {labelType: modalText, itemNumber: this.state.itemNumber})
			})
			this.timedPrint();
		} else {
			alert(printSuccess);
	...
	timedOrderable = () => {
		if(!this.state.itemOrderable) {
			...
			setTimeout(this.toggleIsOrderable, 4000)
		}
	}
	...
	render() {
		...
		return (
			<View style={{flex: 1, width: "100%"}}>
				...
				<ScrollView style={orderStyles.container} keyboardShouldPersistTaps={'handled'} >
					...
					<View style={orderStyles.row}>
						...
						<View style={orderStyles.correct}>
							...
							<Text style={{ fontSize: 13 }}> {translate("quantity_correct")} </Text>
						</View>
					...
					<View style={orderStyles.twoColumn}>
						...
						<View style={orderStyles.smallColumn}>
							...
							<DualRowDisplay title={translate("sc") + ":"} value={this.state.salesCode} />
					...
					<View style={{paddingLeft: 5}}>
						<AwesomeButton 
							...
							textSize={22} width={310} height={45}
							textColor={'white'}
					...
					<Text style={orderStyles.orderBtnText}>{this.state.previousSku}</Text>

				</ScrollView>
				</View>
				<View style={orderStyles.orderPrintPanel}>
					<TouchableNativeFeedback background={TouchableNativeFeedback.Ripple(color.btn_selected)} onPress={this.itemPress}>
						<View style={orderStyles.btn}>
							<Text style={orderStyles.btnText}> {translate("item_btn")} </Text>
						</View>
					</TouchableNativeFeedback>

					<TouchableNativeFeedback background={TouchableNativeFeedback.Ripple(color.btn_selected)} onPress={ this.onPressReview }>
						<View style={orderStyles.btn}>
							<Text style={orderStyles.btnText}> {translate("review_btn")} </Text>
						</View>
					</TouchableNativeFeedback>
				</View>
				...
				<Modal isVisible={this.state.itemOrderable} hasBackdrop={false} coverScreen={false} animationIn="fadeIn" animationOut="fadeOut">
					<View  style={modalStyles.noDbModal}>
						<View style={modalStyles.icon} >
							<Icon name={'alert-circle'} size={50} color={color.modal_red} />
						</View>
						<View style={modalStyles.modalText}>
							<Text style={modalStyles.smallText}>{translate("not_orderable")}</Text>
						</View>
					</View>
				</Modal>
				...
				<Modal isVisible={this.state.isPrintVisible} hasBackdrop={false} coverScreen={false} animationIn="fadeIn" animationOut="fadeOut">
				<View  style={modalStyles.noDbModal}>
					<View style={modalStyles.icon} >
						<Icon name={'printer-wireless'} size={50} color={color.light_green} />
					</View>
					<View style={modalStyles.modalText}>
						<Text style={modalStyles.smallText}>{this.state.printingText}</Text>
					</View>
				</View>
			</Modal>
			</View>
...
export default addHeader(Order, 'Item Ordering')

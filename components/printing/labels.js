...
export default class Label {
    ...
    static async PrintLabel(type, sku, printerAddress) {
        ...
        let success = "";
        success = await this.getResults(type, query);
    ...
    static async getResults(type, query) {
        ...
        if (results.length == 0) {
            ...
            if (type == "shelf") {
                ...
                result = await PrinterConfig.loadForm("shelf");
            } else if (type == "upc") {
                data = UpcLabel.parseResults(results);
                result = await PrinterConfig.loadForm("upc");
            }
